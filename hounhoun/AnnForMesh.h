#ifndef AnnForMesh_h__
#define AnnForMesh_h__
...
public:
	...
	indices(std::vector< size_t >(count)),
		distances(std::vector< coord_t >(count))
	{ }

	typename std::vector< size_t >::iterator indices_it;
	std::vector< size_t > indices;
	typename std::vector< coord_t>::iterator distances_it;
	...
	result_knn findNearest(const PointType& pt) { return findNeighbors(pt, 1); }

	result_knn findNeighbors(const PointType& pt, const size_t numResults = 1)
	{
		result_knn r(numResults);

		coord_t query[3] = { pt[0], pt[1], pt[2] };
	...
	result_radius findWithin(const PointType& pt, const coord_t radius)
	{
		result_radius r;
...
private:
	...
	struct Adapter
	{
		...
		Adapter(const container_t &_container) : container(_container) {}

		inline const container_t & getContainer() const { return container; }
		inline const PointType getPoint(size_t idx) const { return getContainer().at(idx).getPoint();}
...
class AnnForMesh
{
	typedef hccl::TriMesh MeshType;
	typedef MeshType::Point PointType;
	typedef MeshType::Scalar ScalarType;

public:
	AnnForMesh(MeshType* mesh = NULL);
	~AnnForMesh(){}

	void setMesh(MeshType* mesh);

	void updateAnn();
	PointType findNearest(const PointType& pt);
	std::vector<PointType> findNeighbors(const PointType& pt, int num_pt);
	std::vector<PointType> findWidthin(const PointType& pt, ScalarType radius);

	int findNearestIndx(const PointType& pt);
	std::vector<int> findNeighborsIndx(const PointType& pt, int num_pt);
	std::vector<int> findWidthinIndx(const PointType& pt, ScalarType radius);

	int getProjectedPoint(const PointType& p, PointType& proj, ScalarType threshold = 0.5);
	int getProjectedPoint(const PointType& p, const PointType& dir, PointType& proj, ScalarType threshold = 0.5, bool bidirection = true);
...
private:
	...
	{
	public:
		AnnPoint() : point(0,0,0) {}
		AnnPoint(const PointType& pt) : point(pt) {}

		inline const PointType getPoint() const { return point; }
		inline PointType* getPtr() { return &point; }
	private:
		PointType point;
	};

	typedef std::vector< AnnPoint > Points;
	typedef Ann< Points, int, ScalarType > AnnPoints;

	Points allPoints;
	AnnPoints ann_allPoints;

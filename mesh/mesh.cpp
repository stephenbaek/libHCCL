...
bool PolyMesh::read(std::string strFilePath){
    OpenMesh::IO::Options opt;
    if(!has_vertex_normals())
    {
        std::cerr << "File Open Error: Standard vertex property 'Vertex Normals' not available!\n";
...
bool PolyMesh::write(std::string strFilePath) const{
    if ( !OpenMesh::IO::write_mesh(*this, strFilePath) )
    {
        std::cerr << "Cannot write mesh to file" << strFilePath << std::endl;
        return false;
...
void PolyMesh::scale(double s){
    for(PolyMesh::VertexIter vit = vertices_begin(), vend = vertices_end(); vit != vend; ++vit){
        set_point(*vit, point(*vit)*s);
    }    
}
...
void PolyMesh::translate(Vector v)
{
    translate(v[0], v[1], v[2]);
}

// void PolyMesh::rotate( double angle, SNUMesh::Point axis )
// {
// 
// }

Point PolyMesh::calc_center_of_geometry(){
    Point cog(0,0,0);
    if(n_vertices() == 0)
        return cog;
    for(PolyMesh::VertexIter vit = vertices_begin(), vend = vertices_end(); vit != vend; ++vit){
...
double PolyMesh::calc_max_radius(){
    double maxr = 0.0, r;
    for(PolyMesh::VertexIter vit = vertices_begin(), vend = vertices_end(); vit != vend; ++vit){
        r = point(*vit).sqrnorm();
        if(r > maxr)
...
double PolyMesh::calc_min_radius(){
    double minr = INF, r;
    for(PolyMesh::VertexIter vit = vertices_begin(), vend = vertices_end(); vit != vend; ++vit){
        r = point(*vit).sqrnorm();
        if(r < minr)
